...
class KalshiTradingAPI(AbstractTradingAPI):
    ...
    def _get_signed_headers(self, method: str, path: str):
        timestamp = int(time.time() * 1000)
        timestamp_str = str(timestamp)

        # The path passed in will now be complete.
        path_for_signature = "/trade-api/v2" + path
    ...
    def make_request(
        ...
        # The URL for the actual request uses the base path.
        url = f"{self.base_url}{path}"

        # The path for signing must include the encoded query parameters.
        path_for_signing = path
        if params:
            query_string = urlencode(params, quote_via=quote)
            path_for_signing = f"{path}?{query_string}"

        # Generate headers using the full path with the query string.
        headers = self._get_signed_headers(method, path_for_signing)
        ...
        try:
            ...
            # The 'params' dict is handled correctly by the requests library.
...
class AvellanedaMarketMaker:
    ...
    def calculate_optimal_spread(self, t: float, inventory: int) -> float:
        ...
        final_spread = max(base_spread * spread_adjustment, self.min_spread)
...
class KalshiTradingAPI(AbstractTradingAPI):
    ...
    def _load_private_key(self):
        try:
            ...
            if '\n' not in private_key_content and '-----BEGIN RSA PRIVATE KEY-----' in private_key_content:
                self.logger.info("Fixing private key formatting (adding newlines)")
                content = private_key_content.replace('-----BEGIN RSA PRIVATE KEY-----', '').replace('-----END RSA PRIVATE KEY-----', '').strip()
    ...
    def get_position(self) -> int:
        ...
        try:
            response = self.make_request("GET", path, params=params)
            positions = response.get("market_positions", [])
            total_position = sum(p["position"] for p in positions if p["ticker"] == self.market_ticker)
            self.logger.info(f"Current position: {total_position}")
            return total_position
        except Exception as e:
            self.logger.warning(f"Failed to get position (using fallback): {e}")
    ...
    def place_order(self, action: str, side: str, price: float, quantity: int, expiration_ts: int = None) -> str:
        ...
        path = "/portfolio/orders"
        data = {
            "ticker": self.market_ticker, "action": action.lower(), "type": "limit",
            "side": side, "count": quantity, "client_order_id": str(uuid.uuid4()),
        }
        price_cents = int(round(price * 100))
        if side == "yes": data["yes_price"] = price_cents
        else: data["no_price"] = price_cents
        if expiration_ts is not None: data["expiration_ts"] = expiration_ts
        try:
            response = self.make_request("POST", path, data=data)
            order_id = response["order"]["order_id"]
            self.logger.info(f"Successfully placed {action} order: ID {order_id}")
...
class AvellanedaMarketMaker:
    def __init__(
        self, logger: logging.Logger, api: AbstractTradingAPI, gamma: float, k: float,
        sigma: float, T: float, max_position: int, order_expiration: int,
        min_spread: float = 0.01, position_limit_buffer: float = 0.1,
        inventory_skew_factor: float = 0.01, trade_side: str = "yes"
    ):
        self.api = api
        self.logger = logger
        self.base_gamma = gamma
    ...
    def run(self, dt: float):
        ...
        iteration = 0
        run_id = os.getenv('RUN_ID', 'unknown')
        self.logger.info(f"[{run_id}] Starting Avellaneda market maker for {self.T/3600:.1f} hours")
        while time.time() - start_time < self.T:
            current_time = time.time() - start_time
            iteration += 1
            self.logger.info(f"=== Iteration {iteration} at {current_time:.1f}s ===")
            try:
                ...
                bid_price, ask_price = self.calculate_asymmetric_quotes(mid_price, inventory, current_time)
                buy_size, sell_size = self.calculate_order_sizes(inventory)
                self.logger.info(f"Calculated quotes - Reservation: ${reservation_price:.3f}, Bid: ${bid_price:.3f}, Ask: ${ask_price:.3f}")
                self.manage_orders(bid_price, ask_price, buy_size, sell_size)
            except Exception as e:
                self.logger.error(f"Error in market making iteration: {e}")
            time.sleep(dt)
    ...
    def calculate_optimal_spread(self, t: float, inventory: int) -> float:
        if self.T <= 0: return self.min_spread
        dynamic_gamma = self.calculate_dynamic_gamma(inventory)
        time_factor = max(0, 1 - t/self.T)
        try:
            base_spread = (dynamic_gamma * (self.sigma**2) * time_factor + (2 / dynamic_gamma) * math.log(1 + (dynamic_gamma / self.k)))
        except (ZeroDivisionError, ValueError):
            base_spread = self.min_spread
        position_ratio = abs(inventory) / self.max_position if self.max_position > 0 else 0
        spread_adjustment = 1 - (position_ratio ** 2)
        final_spread = max(base_spread * spread_adjustment, self.min_spread)
        return min(final_spread, 0.20)

    def calculate_dynamic_gamma(self, inventory: int) -> float:
        if self.max_position <= 0: return self.base_gamma
        position_ratio = inventory / self.max_position
        return self.base_gamma * math.exp(-abs(position_ratio))
    ...
    def manage_orders(self, bid_price: float, ask_price: float, buy_size: int, sell_size: int):
        try:
            current_orders = self.api.get_orders()
            self.logger.info(f"Managing orders - Found {len(current_orders)} existing orders")
            buy_orders = [o for o in current_orders if o.get('side') == self.trade_side and o.get('action') == 'buy']
            sell_orders = [o for o in current_orders if o.get('side') == self.trade_side and o.get('action') == 'sell']
            self.logger.info(f"Current orders - Buy: {len(buy_orders)}, Sell: {len(sell_orders)}")
            self.handle_order_side('buy', buy_orders, bid_price, buy_size)
            self.handle_order_side('sell', sell_orders, ask_price, sell_size)
        except Exception as e:
    ...
    def handle_order_side(self, action: str, orders: List[Dict], desired_price: float, desired_size: int):
        ...
        for order in orders:
            try:
                price_key = 'yes_price' if self.trade_side == 'yes' else 'no_price'
                current_price = float(order.get(price_key, 0)) / 100
                remaining_size = order.get('remaining_count', 0)
                if (keep_order is None and abs(current_price - desired_price) < tolerance and remaining_size == desired_size):
                    keep_order = order
                    self.logger.info(f"Keeping {action} order: ID {order['order_id']}, Price ${current_price:.3f}, Size {remaining_size}")
                else:
                    self.logger.info(f"Canceling {action} order: ID {order['order_id']}, Price ${current_price:.3f} (target: ${desired_price:.3f})")
                    self.api.cancel_order(order['order_id'])
            except Exception as e:
                self.logger.error(f"Error processing {action} order {order.get('order_id', 'unknown')}: {e}")
        if keep_order is None:
            try:
                current_prices = self.api.get_price()
                current_market_price = current_prices[self.trade_side]
                should_place = (action == 'buy' and desired_price < current_market_price) or \
                               (action == 'sell' and desired_price > current_market_price)
                if should_place and 0.001 < desired_price < 0.999:
                    order_id = self.api.place_order(
                        action, self.trade_side, desired_price, desired_size,
                        int(time.time()) + self.order_expiration
                    )
                    ...
                    self.logger.info(f"Skipped {action} order: Price ${desired_price:.3f} doesn't improve on market ${current_market_price:.3f}")
            except Exception as e:
                self.logger.error(f"Failed to place {action} order: {e}")

# --- NEW SIMPLE MARKET MAKER ---
class SimpleMarketMaker:
    """
    A simple fixed-spread market maker.
    """
    def __init__(
        self,
        logger: logging.Logger,
        ...
        fixed_spread: float,
        order_expiration: int,
        inventory_skew_factor: float, # Add this
        position_limit_buffer: float = 0.1
    ):
        self.api = api
        self.logger = logger
        ...
        self.order_expiration = order_expiration
        self.inventory_skew_factor = inventory_skew_factor # Add this
        self.position_limit_buffer = position_limit_buffer

    def calculate_asymmetric_quotes(self, fair_value: float, inventory: int, t: float) -> Tuple[float, float]:
        """
        Calculates bid and ask prices based on a fixed spread, skewed by inventory.
        """
        self.logger.info(f"✅ Calculating quotes using fair_value: ${fair_value:.4f}")
        # Skew the mid-price based on our current inventory
        inventory_skew = inventory * self.inventory_skew_factor
        adjusted_mid_price = fair_value - inventory_skew

        half_spread = self.fixed_spread / 2
        bid_price = adjusted_mid_price - half_spread
        ask_price = adjusted_mid_price + half_spread

        # Apply hard inventory limits
        if inventory >= self.max_position:
            self.logger.warning(f"At max long position ({inventory}), stopping bids.")
            bid_price = 0  # Stop buying
        if inventory <= -self.max_position:
            self.logger.warning(f"At max short position ({inventory}), stopping asks.")
            ask_price = 0  # Stop selling

        # Ensure prices are within valid Kalshi range
        bid_price = max(0.001, bid_price)
        ask_price = min(0.999, ask_price)

        return round(bid_price, 3), round(ask_price, 3)

    # --- GENERIC METHODS COPIED FROM AVELLANEDA MARKET MAKER ---
